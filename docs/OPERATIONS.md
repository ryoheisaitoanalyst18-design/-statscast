...
- **既知の継続 WARN**: `run_expectancy.json` の想定外状態キー（負値のボールカウントや
  ストライク数になる外れ値行由来）が現在 3 件あり、WARN が継続して発生する。
  パイプライン側 CSV の外れ値行が原因のため、このリポジトリ単体では修正不可。
  デプロイを止めない既知事項として扱い、**WARN 3 件 (run_expectancy のみ) なら正常**と
  判断してよい。件数が増えた・別ファイルで WARN が出た場合は改めて調査する。
