...
- **着手前に `gh pr list --state open` で未マージ PR を確認する**。ゼロ文脈で毎晩起動するため、
  過去の自分の PR と重複しやすい。同じ対象・同じ意図の PR があれば着手しない。
  未マージが 3 件以上なら新規 PR を作らず、滞留一覧を報告して終了する (FAIL 対応は例外)。
  ブランチは必ず最新の `origin/main` から切る (未マージ PR の上に積むとコンフリクトの山になる)。
